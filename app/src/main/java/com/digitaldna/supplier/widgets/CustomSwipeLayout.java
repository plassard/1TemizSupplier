...
public class CustomSwipeLayout extends ViewGroup {
    ...
    private final ViewDragHelper.Callback dragCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            swipeListener.onUpdate(CustomSwipeLayout.this, left);
            ...
            if (dx > 0) {
                ...
                if (leftView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (leftView.getRight() - stickyBound > 0 && leftView.getRight() - stickyBound - dx <= 0)
                            swipeListener.onLeftStickyEdge(CustomSwipeLayout.this, true);
                    }
                }
                ...
                if (rightView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (rightView.getLeft() + stickyBound > getWidth() && rightView.getLeft() + stickyBound - dx <= getWidth())
                            swipeListener.onRightStickyEdge(CustomSwipeLayout.this, true);
                    }
                }
            } else if (dx < 0) {
                //move to left
                ...
                if (leftView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (leftView.getRight() - stickyBound <= 0 && leftView.getRight() - stickyBound - dx > 0)
                            swipeListener.onLeftStickyEdge(CustomSwipeLayout.this, false);
                    }
                }
                ...
                if (rightView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (rightView.getLeft() + stickyBound <= getWidth() && rightView.getLeft() + stickyBound - dx > getWidth())
                            swipeListener.onRightStickyEdge(CustomSwipeLayout.this, false);
                    }
                }
            }
        }
        ...
        private boolean onMoveRightReleased(View child, int dx, float xvel) {
            ...
            if (dx > 0 && xvel >= 0 && leftViewClampReached(lp)) {
                if (swipeListener != null) {
                    swipeListener.onSwipeClampReached(CustomSwipeLayout.this, true);
                }
                return true;
            }
        ...
        private boolean onMoveLeftReleased(View child, int dx, float xvel) {
            ...
            if (dx < 0 && xvel <= 0 && rightViewClampReached(lp)) {
                if (swipeListener != null) {
                    swipeListener.onSwipeClampReached(CustomSwipeLayout.this, false);
                }
                return true;
            }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CustomSwipeLayout);
            if (a.hasValue(R.styleable.CustomSwipeLayout_swipe_enabled)) {
                leftSwipeEnabled = a.getBoolean(R.styleable.CustomSwipeLayout_swipe_enabled, true);
                rightSwipeEnabled = a.getBoolean(R.styleable.CustomSwipeLayout_swipe_enabled, true);
            }
            if (a.hasValue(R.styleable.CustomSwipeLayout_left_swipe_enabled)) {
                leftSwipeEnabled = a.getBoolean(R.styleable.CustomSwipeLayout_left_swipe_enabled, true);
            }
            if (a.hasValue(R.styleable.CustomSwipeLayout_right_swipe_enabled)) {
                rightSwipeEnabled = a.getBoolean(R.styleable.CustomSwipeLayout_right_swipe_enabled, true);
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (MeasureSpec.getMode(heightMeasureSpec) == MeasureSpec.EXACTLY) {
            ...
            if (maxHeight > 0) {
                int heightMeasureSpecTemp = MeasureSpec.makeMeasureSpec(maxHeight, MeasureSpec.EXACTLY);
                measureChildren(widthMeasureSpec, heightMeasureSpecTemp);
            }
        }
    ...
    private void startScrollAnimation(View view, int targetX, boolean moveToClamp, boolean toRight) {
        if (dragHelper.settleCapturedViewAt(targetX, view.getTop())) {
            ...
            if (moveToClamp && swipeListener != null) {
                swipeListener.onSwipeClampReached(CustomSwipeLayout.this, toRight);
            }
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (touchState == TOUCH_STATE_WAIT) {
                    ...
                    if (dx >= touchSlop || dy >= touchSlop) {
                        touchState = (Float.compare(dy, 0F) == 0 || dx / dy > 1f) ? TOUCH_STATE_SWIPE : TOUCH_STATE_SKIP;
            ...
            case MotionEvent.ACTION_UP:
                if (touchState == TOUCH_STATE_SWIPE) {
                    if (swipeListener != null) {
                        swipeListener.onEndSwipe(CustomSwipeLayout.this);
                    }
                    unHackParents();
    ...
    public interface OnSwipeListener {
        void onBeginSwipe(CustomSwipeLayout swipeLayout, boolean moveToRight);

        void onSwipeClampReached(CustomSwipeLayout swipeLayout, boolean moveToRight);

        void onLeftStickyEdge(CustomSwipeLayout swipeLayout, boolean moveToRight);

        void onRightStickyEdge(CustomSwipeLayout swipeLayout, boolean moveToRight);

        void onUpdate(CustomSwipeLayout swipeLayout, int left);

        void onEndSwipe(CustomSwipeLayout swipeLayout);
    }
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs, R.styleable.CustomSwipeLayout);

            final int N = a.getIndexCount();
            for (int i = 0; i < N; ++i) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.CustomSwipeLayout_gravity) {
                    gravity = a.getInt(attr, CENTER);
                } else if (attr == R.styleable.CustomSwipeLayout_sticky) {
                    sticky = a.getLayoutDimension(attr, STICKY_SELF);
                } else if (attr == R.styleable.CustomSwipeLayout_clamp) {
                    clamp = a.getLayoutDimension(attr, CLAMP_SELF);
                } else if (attr == R.styleable.CustomSwipeLayout_bring_to_clamp) {
                    bringToClamp = a.getLayoutDimension(attr, BRING_TO_CLAMP_NO);
                } else if (attr == R.styleable.CustomSwipeLayout_sticky_sensitivity) {
    ...
    private class SettleRunnable implements Runnable {
        ...
        public void run() {
            if (dragHelper != null && dragHelper.continueSettling(true)) {
                ...
                if (moveToClamp && swipeListener != null) {
                    swipeListener.onSwipeClampReached(CustomSwipeLayout.this, moveToRight);
                }
            }
        }
    }
